...
glm::vec3 PathIntegrator::SampleRR(const Ray &ray) const {
  using namespace glm;
  ...
  return scene_.Trace(
      ...
      [&](const RayHit &hit) {
        ...
        const auto w_o = -ray.direction;
        return LightDirect(x, n, w_o, brdf) +
               LightIndirectRR(x, n, w_o, brdf, vec3(1.f));
      });
}
...
glm::vec3 PathIntegrator::LightIndirectRR(const glm::vec3 &x,
                                          const glm::vec3 &n,
                                          const glm::vec3 &w_o,
                                          const BRDF &brdf,
  ...
  return scene_.Trace(
      ...
      [&](const RayHit &hit) {
        ...
        const auto throughput_RR = 2 * glm::pi<float>() * brdf(w_i, w_o) *
                                   dot(n, w_i) * throughput / (1 - q);
        return LightDirect(x_i, n_i, -w_i, brdf_i) * throughput_RR +
               LightIndirectRR(x_i, n_i, -w_i, brdf_i, throughput_RR);
      });
}
